...
function computeCurve(cardData) {
  ...
  cardData.forEach((card) => {
    if (!card || typeof card.cmc !== "number" || card.cmc < 0) return;
...
function detectSynergies(cardData) {
  ...
  if (textSnippets.includes("gain") && textSnippets.includes("life")) synergies.push("Lifegain");
...
function tagCardRoles(cardData) {
  ...
  cardData.forEach((card) => {
    ...
    if (text.includes("lifelink") || (text.includes("gain") && text.includes("life"))) roles.push("lifegain");
...
async function analyzeMatchups(deckCards) {
  const { fetchMTGGoldfishMeta, fetchScryfallArchetypes, fetchMTGTop8Meta, generateMetaProfile } = await import("./metaSources.js");
