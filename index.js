...
import { analyzeMatchups, analyzeSideboard, analyzeMainDeck } from "./analyzeMatchups.js";
...
app.post("/api/analyze-deck", async (req, res) => {
  ...
  try {
    ...
    // Analyze the main deck (basic analysis)
    const mainDeckAnalysis = await analyzeMatchups(mainDeck);
    
    // Detailed main deck analysis
    const detailedMainDeckAnalysis = await analyzeMainDeck(mainDeck, mainDeckAnalysis);
    ...
    res.json({ 
      mainDeck: detailedMainDeckAnalysis,
